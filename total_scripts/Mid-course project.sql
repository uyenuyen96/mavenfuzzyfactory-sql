...
/* 3. While we’re on Gsearch, could you dive into nonbrand,
   ...
   I want to flex our analytical muscles a little and show the board
   we really know our traffic sources.
 */
 ...
 */
-- first, finding various utm sources and references to see the traffic we're getting
SELECT DISTINCT
    utm_source,
    utm_campaign,
    http_referer
...
/* 5. I’d like to tell the story of our website performance improvements
   over the course of the first 8 months. Could you pull session to order
...
USE mavenfuzzyfactory;
SELECT
    MIN(website_pageview_id) AS first_pv_id -- 23504
FROM
    website_pageviews
...
-- then we make a table t bring orders
...
-- to find the differences between conversion rate
SELECT
    landing_page,
...
/* 7. For the landing page test you analyzed previously,
   it would be great to show a full conversion funnel from each of the
   two pages to orders. You can use the same time period you analyzed last time (Jun 19 – Jul 28).
 */
SELECT
...
-- then this would produce e final output, part 1
SELECT
    CASE
        WHEN saw_homepage = 1 THEN 'saw_homepage'
        WHEN saw_custom_lander = 1 THEN 'saw_custom_lander'
        ELSE 'uhm..check logic'
    END AS segment,
    COUNT(DISTINCT website_session_id) AS sessions,
...
-- then this as final output part 2 - click rate
SELECT
    CASE
        WHEN saw_homepage = 1 THEN 'saw_homepage'
        WHEN saw_custom_lander = 1 THEN 'saw_custom_lander'
...
/* 8. I’d love for you to quantify the impact of our billing test, as well.
   ...
   in terms of revenue per billing page session, and then pull the number of billing page
...
FROM
 ...
 SELECT
     website_pageviews.website_session_id,
     website_pageviews.pageview_url AS billing_verion_seen,
     orders.order_id,
     orders.price_usd
FROM website_pageviews
    LEFT JOIN orders
...
-- uplift $8.51 per billing session
...
-- VALUE OF BILLING TEST: =$8.51*1193 = $10,152 over the past month